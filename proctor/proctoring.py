...
def get_landmark_model():
    cwdlm = os.path.abspath(os.path.dirname(__file__))
    pose_model_directory = os.path.abspath(os.path.join(cwdlm, "pose_model"))
    saved_model_path = os.path.join(pose_model_directory, "saved_model.pb")
    converted_model_path = os.path.join(pose_model_directory, "converted_model.h5")
    
    model = tf.saved_model.load(pose_model_directory)
...
def get_analysis(imgData,gaze_model_path):
    ...
    for i in range(nums[0]):
        ...
        if int(classes[0][i] == 67):
            mob_status = 'Handphone Terdeteksi'
        else:
            mob_status = 'Handphone Tidak terdeteksi'

    if count == 0:
        person_status = 'No person detected'
    elif count > 1:
        person_status = 'Lebih dari 1 orang terdeteksi'
    else:
    ...
    if gaze.is_blinking():
        eye_movements = "Ngedip"
    elif gaze.is_right():
        eye_movements = "Lihat ke kanan"
    elif gaze.is_left():
        eye_movements = "Lihat ke kiri"
    elif gaze.is_center():
        eye_movements = "Lihat ke depan"
    else:
